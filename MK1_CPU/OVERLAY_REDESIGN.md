...
| 2026-04-22 | Phase A overlay-mode inline split + cost-benefit guard | `_split_helpers_for_overlay` runs inside `_overlay_partition` after T2.1 and before kernel sizing. Bodies of helpers in `HELPER_OVERLAY_NAMES` that land in `runtime_resident_helpers` (the natural classification set) are replaced with 4B thunks and `_load_helper` is appended. `OVERLAY_REGION` then sizes correctly against the reduced kernel. The ceiling check at line ~5778 and the final `avail = 250 - OVERLAY_REGION` shrink by `R_HELPER_SIZE` so user overlays cannot extend into R_helper. Post-partition emission (`_emit_helper_overlay_bodies`) pre-assembles bodies with external labels resolved from a full-program assembly pass, then writes raw `byte` directives in page3 — manifest entries (`byte __h_body_N; byte size`) resolve correctly at the assembler's second pass. **Cost-benefit guard** in `_split_helpers_for_overlay`: reject when `len < 2` or `kernel_save <= overlay_cost`. Single-helper extraction paid 28B loader + max(body) reserve to save `body - 4B` — measured -32B net on lcd_temp_overlay, so the guard rejects it. The intended benefit (multi-helper dedup from bundled overlays) requires the FOLLOWUP step of extracting bundled helpers too, which the current split only looks for in resident helpers. Eight of 11 failing programs still fail — they need helper *extraction from bundled overlays*, not just from the resident set. Next milestone: widen the split to scan overlay bodies for `__lcd_chr:`/`__print_u8_dec:` occurrences (bundled copies) and replace them with kernel thunks + single R_helper body. |
